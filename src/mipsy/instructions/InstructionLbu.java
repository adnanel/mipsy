...
public class InstructionLbu extends Instruction {
    private String destReg;
    private int offset;
    private String baseReg;

    public InstructionLbu(List<String> args) {
        super(args);

        this.instruction = "lbu";

        if ( args.size() != 2 )
            throw new IllegalArgumentException("Invalid arguments passed to lbu! Expected 2, given " + args.size());
    ...
    public String toString() {
        return String.format("lbu %s, %d(%s)", destReg, offset, baseReg);
    }
}
