...
public class InstructionLl extends Instruction {
    private String destReg;
    private int offset;
    private String baseReg;

    public InstructionLl(List<String> args) {
        super(args);

        this.instruction = "ll";

        if ( args.size() != 2 )
            throw new IllegalArgumentException("Invalid arguments passed to ll! Expected 2, given " + args.size());
    ...
    public String toString() {
        return String.format("ll %s, %s(%s)", destReg, "0x" + Integer.toHexString(offset), baseReg);
    }
}
