...
public class ID extends DataPhase {
    ...
    public void step(Consumer<String> logger) {
        ...
        logger.accept(String.format("ID: Sending IF_OUT0 (%s) to ID_OUT0", Integer.toHexString(prev.IF_OUT0)));
        ID_OUT0 = prev.IF_OUT0;

        logger.accept(String.format("ID: Sending IF_OUT1[25:21] (%s) to ReadReg1", Integer.toHexString(Utility.SubBits(prev.IF_OUT0, 21, 26))));
        registersComponent.setReadRegister1( Utility.SubBits(prev.IF_OUT1, 21, 26) );

        logger.accept(String.format("ID: Sending IF_OUT1[20:16] (%s) to ReadReg2 and MUX1, input 0", Integer.toHexString(Utility.SubBits(prev.IF_OUT1, 16, 21))));
        ...
        logger.accept(String.format("ID: Sending read data 1 (%s) to ID_OUT1", Integer.toHexString(readData1)));
        ID_OUT1 = readData1;

        int readData2 = registersComponent.getReadData2(logger).value;
        logger.accept(String.format("ID: Sending read data 2 (%s) to ID_OUT3", Integer.toHexString(readData2)));
        ID_OUT3 = readData2;

        int n = Utility.SubBits(prev.IF_OUT1, 11, 16);
        logger.accept(String.format("ID: Sending IF_OUT1[15:11] (%s) to MUX1, input 1", Integer.toHexString(n)));
        mux1.setB( n );

        int regdst = core.controlComponent.getRegDst();
        logger.accept(String.format("ID: Sending RegDst (%s) to MUX1", Integer.toHexString(regdst)));
        mux1.setSelector(regdst);

        int regwrite = core.controlComponent.getRegWrite();
        logger.accept(String.format("ID: Sending RegWrite (%s) to RegisterComponent(RegWrite)", Integer.toHexString(regwrite)));
        ...
        logger.accept(String.format("ID: Sending IF_OUT1[15:0] (%s) to SignExtend and ID_OUT4", Integer.toHexString(Utility.SubBits( prev.IF_OUT1, 0, 16 ))));
        ID_OUT4 = Utility.SubBits( prev.IF_OUT1, 0, 16 );

        int seout = SignExtendComponent.extend( ID_OUT4 );
        logger.accept(String.format("ID: Sending SignExtend output (%s) to ID_OUT2", Integer.toHexString(seout) ) );
        ID_OUT2 = seout;
    }
}
