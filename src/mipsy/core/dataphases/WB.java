...
public class WB extends DataPhase {
    ...
    public void step(Consumer<String> logger) {
        ...
        logger.accept(String.format("WB: Sending MEM_OUT1 (%s) to MUX4", "0x" + Integer.toHexString(prev.MEM_OUT1)));
        mux4.setB(prev.MEM_OUT1);

        logger.accept(String.format("WB: Sending MEM_OUT2 (%s) to MUX4", "0x" + Integer.toHexString(prev.MEM_OUT2)));
        mux4.setA(prev.MEM_OUT2);

        int res = core.controlComponent.getMemToReg();
        logger.accept(String.format("WB: Sending MemToReg (%s) to MUX4", "0x" + Integer.toHexString(res)));
        mux4.setSelector(res);

        logger.accept(String.format("WB: Sending MEM_OUT0 (%s) output to PC", "0x" + Integer.toHexString(prev.MEM_OUT0)));
        ifPhase.pc = prev.MEM_OUT0;

        int mux4Out = mux4.getResult(logger);
        logger.accept(String.format("WB: Sending MUX4 output (%s) to Registers", "0x" + Integer.toHexString(mux4Out)));
